...
Something interesting about “The Raven” is its use of dialogue. How many lines of dialogue (aka sets of "" marks) are present in the entire poem? To find out, we’ll need to do the following:
...
/* PART 2: Counting Specific Substring Words (Counting NEVERMORE)
...
class Count {
  public static int findNumValues(String text, String findText) {
    ...
    // In order to prevent an indexOutOfBounds error, we need to start the loop at the beginning of the String, but end the iteration before the loop control variable is equal to the length of the String minus the length of the String value we are searching for.
    for (int i=0; i<text.length() - findText.length(); i++){

      // check if the current element and the next several element values are equivalent to the text we are searching for. If it is, increase the counter variable by 1.
